...
from pypoplib.cmaes import CMAES
from pypoplib.cc import CC
    ...
    def optimize(self, fitness_function=None, args=None):
        ...
        while not self._check_terminations():
            if is_initialization:
                ...
                y = []
                for i, opt in enumerate(sub_optimizers):
                    ii = range(i*self.ndim_subproblem, int(np.minimum((i + 1)*self.ndim_subproblem, self.ndim_problem)))
                    if self._check_terminations():
                        break

                    def sub_function(sub_x, s, r):  # to define sub-function for each sub-optimizer
                        best_so_far_x = np.copy(self.best_so_far_x)
                        best_so_far_x[ii] = sub_x
                        return self.fitness_function(best_so_far_x, s, r)
                    ...
                    y.extend(y_s[i])
                    if opt.best_so_far_y < self.best_so_far_y:
                        self.best_so_far_x[ii], self.best_so_far_y = opt.best_so_far_x, opt.best_so_far_y
                    self.time_function_evaluations += opt.time_function_evaluations
                    self.n_function_evaluations += opt.n_function_evaluations
            self._n_generations += 1
        results = self._collect(fitness, y)
        ee, eee = [], []
        for i in range(len(sub_optimizers)):
            ee.append(ee_s[i][:, int(np.argsort(ea_s[i])[-1])])
        for e in ee:
            eee = np.hstack((eee, e))
        results['ee'] = eee
        return results
